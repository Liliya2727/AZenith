...
static void load_initial_config_files(DaemonContext* ctx) {
    ...
    if ((fp = fopen("/data/adb/.config/AZenith/bypasschgconfig/bypasschgthreshold", "r"))) {
        if (fgets(val, sizeof(val), fp)) ctx->config_bypasschgthreshold = atoi(val);
        fclose(fp);
    }
    ...
    while ((ptr = strstr(ptr, "\": {")) != NULL) {
        ...
        if (*start_quote == '"') {
            ...
            if (p && (!next_block || p < next_block)) {
                extract_string_value(g_game_cache[g_game_cache_count].perf_lite_mode, p, sizeof(g_game_cache[g_game_cache_count].perf_lite_mode));
            } else strcpy(g_game_cache[g_game_cache_count].perf_lite_mode, "default");

            p = strstr(ptr, "\"dnd_on_gaming\":");
            if (p && (!next_block || p < next_block)) {
                extract_string_value(g_game_cache[g_game_cache_count].dnd_on_gaming, p, sizeof(g_game_cache[g_game_cache_count].dnd_on_gaming));
            } else strcpy(g_game_cache[g_game_cache_count].dnd_on_gaming, "default");

            p = strstr(ptr, "\"app_priority\":");
            if (p && (!next_block || p < next_block)) {
                extract_string_value(g_game_cache[g_game_cache_count].app_priority, p, sizeof(g_game_cache[g_game_cache_count].app_priority));
            } else strcpy(g_game_cache[g_game_cache_count].app_priority, "default");

            p = strstr(ptr, "\"game_preload\":");
            if (p && (!next_block || p < next_block)) {
                extract_string_value(g_game_cache[g_game_cache_count].game_preload, p, sizeof(g_game_cache[g_game_cache_count].game_preload));
            } else strcpy(g_game_cache[g_game_cache_count].game_preload, "default");

            p = strstr(ptr, "\"refresh_rate\":");
            if (p && (!next_block || p < next_block)) {
                extract_string_value(g_game_cache[g_game_cache_count].refresh_rate, p, sizeof(g_game_cache[g_game_cache_count].refresh_rate));
            } else strcpy(g_game_cache[g_game_cache_count].refresh_rate, "default");
            ...
            } else strcpy(g_game_cache[g_game_cache_count].renderer, "default");
...
int main_daemon(void) {
    ...
    while (1) {
        ...
        if (real_screen_state != ctx.prev_screen_state) {
            if (real_screen_state == 0) {
                ...
                if (ctx.grace_period_active) {
                    log_zenith(LOG_INFO, "Screen ON Event: Grace period aborted. Keeping Performance Profile.");
                    ctx.grace_period_active = false;
                }
                ctx.screen_off_timer = 0;
                ctx.need_profile_checkup = true;
        ...
        if (ctx.is_initialize_complete && ctx.cur_mode != PERFORMANCE_PROFILE && !pending_game_pid && !ctx.need_profile_checkup) {
            continue;
        }
